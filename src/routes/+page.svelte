<script lang="ts">
	...
	const handleLocationSelect = async (code: string, name: string, subName: string) => {
		isNavigating = true;
...
<header class="sticky top-0 z-30 border-b border-white/5 bg-bg-base/80 backdrop-blur-xl">
	<div class="mx-auto flex max-w-4xl items-center gap-2 px-4 py-3 sm:px-6">
		<!-- Brand -->
		<a href="/" class="flex items-center gap-2 shrink-0 transition-opacity hover:opacity-80">
			<img src="/logo.svg" alt="Meteoid Logo" class="h-6 w-6" />
			<span class="hidden text-base font-bold tracking-tight text-text-primary sm:block">Meteoid</span>
		</a>

		<!-- Location display (clickable to open drawer) -->
		<button
			onclick={() => (drawerOpen = true)}
			class="flex flex-1 min-w-0 items-center gap-2 rounded-xl border border-white/8 bg-white/4 px-3 py-2
		...
		>
			<svg class="h-3.5 w-3.5 shrink-0 text-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor">
				<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
					d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
				<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
			</svg>
			<div class="min-w-0 flex-1">
				<div class="truncate text-sm font-medium leading-tight text-text-primary">
					{locationLabel()}
				</div>
				{#if locationSubLabel()}
		...
		>
			<svg class="h-4 w-4 {isNavigating ? 'animate-spin' : ''}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
				<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
